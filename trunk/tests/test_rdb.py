...
def test():
    ...
    if db1.db != db3.db:
	print 'Update mismatch:\n  insert: %s\n  update: %s' % (db3.db, db1.db)
	ok = 0
	
    return ok


# The same test using plxlib, which in turns calls the C Xlib, for
# reference.  However, it doesn't seem to grok the wildcard '?' which
# gives four errors on the name vs class vs ? test...

def plxlibtest():
    from plxlib import plxlib
    
    db = plxlib.GetStringDatabase(resources)
    ok = 1
    for name, cls, value in queries:
	try:
	    v = db.GetStringResource(name, cls)
	    if v != value:
		print 'Value mismatch for %s/%s:\n    expected %s, got %s' % (name, cls, value, v)
		ok = 0
